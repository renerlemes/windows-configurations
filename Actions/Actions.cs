...
namespace Windows.Configurations
{
    public static class Actions
    {
        ...
        public static IWindowsAction[] All() =>
        [
            PainelControle.DisableUac,
            PainelControle.NoSoundScheme,
            PainelControle.DisableStartupSound,
            PainelControle.LidCloseDoNothing,
            PainelControle.NeverSleepOrTurnOffDisplay,
            Personalizacao.TaskbarAlignAndSettings,
            Personalizacao.DisableItemsTaskbar
        ];

        /// <summary>
        /// O nome do tipo identifica a ação na linha de comando da instância elevada.
        /// </summary>
        public static IWindowsAction Find(string typeName)
        {
            foreach (IWindowsAction action in All())
            {
                if (string.Equals(action.GetType().Name, typeName, StringComparison.OrdinalIgnoreCase))
                    return action;
            }
